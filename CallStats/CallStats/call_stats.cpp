...
Description: Extension of assignment 1, A program that calculates the net cost of a call (net_cost), the
tax on a call (call_tax) and the total cost of that call (total_cost). Displays the calls Info. Uses three functions, input, output, and process. Utilizes a file data stream.
*/
...
///*************************************************************************************
//Name:  Output
//Precondition: The variables cell_number, relays, call_length have been initialized and set to constants and net_cost, call_tax, and total_cost_of_call have their constant values initialized as well based on what was retrieved from the file.
//Postcondition: cout precision has been set and output from the customer_record has been displayed to the screen.
//Description:  Sets the format of cout, and outputs from customer_record onto the screen.
//*************************************************************************************
...
void Output(const call_record & customer_record)
{
	...
	cout << customer_record.cell_number << "\t" << customer_record.relays << "\t" << customer_record.call_length << "\t" << customer_record.net_cost << "\t" << customer_record.tax_rate << "\t" << customer_record.call_tax << "\t" << customer_record.total_cost << endl << endl;
}
